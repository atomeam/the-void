...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 w-[min(92vw,32rem)] -translate-x-1/2 -translate-y-1/2 rounded-xl bg-surface p-6 pt-7 shadow-[var(--shadow-border)]",
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-3 top-3 size-9 rounded-sm text-muted hover:bg-fg/6 hover:text-fg inline-flex items-center justify-center">
        <X className="size-4" />
